...
updateMatch f = let foo res = return (Just (f res, ()))
                in MatchState foo

getMatchRes :: MatchState MatchRes
getMatchRes = let foo res = return (Just (res, res))
              in MatchState foo
...
internalReplace val@(Atom name) ms = fromMaybe val (M.lookup name ms)
internalReplace (List ls) ms = List $ map (`internalReplace` ms) ls
...
makeBlankTest name (List (Atom matched:_)) = fromBool (name == matched)
...
parseBlank (List [Atom "Blank"]) = Just (Single Blank)
parseBlank (List [Atom "Blank", Atom y]) = Just
  (WithTest (makeBlankTest y) (Single Blank))
...
parseBlankSeq (List [Atom x]) = fmap Single $ lookup x blankCons
...
parsePattern (List [Atom "Pattern", Atom name, pattern]) = fmap (Bind name)
  $ parsePatt pattern
parsePattern _ = Nothing

patternTestType :: ParsedPatt -> PattTest -> PattTest
patternTestType pp f = case parsePatternType pp of
  One -> f
  _   -> mapSequence f

parsePatternTest :: ParseLispval
parsePatternTest (List [Atom "PatternTest", p, f]) = do
  parsed <- parsePatt p
...
patternMatching (WithTest test p) l = let allP = allPossibleMatch p l
                                      in tryMatchList (map (>> test l) allP)
patternMatching (Alt alts) l = matchAlt alts l
patternMatching (Bind name p) l = do
  patternMatching p l
  addNewMatch name l
patternMatching (Then ps ts) (List ls) = matchThen ps ts ls
