...
unpackSlot vs [n] = do
  n' <- unpackSlotNum n
  if length vs < n' + 1
    then throwError (SlotError (head vs))
    else return $ vs !! n'
...
replaceSlot vs (List val@(Atom "Slot":inds)) = unpackSlot vs inds
replaceSlot vs (List val@(Atom "SlotSequence":inds)) = unpackSlotSeq vs inds
replaceSlot _ val@(List (Atom "Function":_)) = return val
replaceSlot vs (List lis) = List <$> mapM (replaceSlot vs) lis
...
unpackPara (List (Atom "List":paras)) vals =
  ...
  then throwError fpct
  else return $ zip (map unpackAtom paras) vals
